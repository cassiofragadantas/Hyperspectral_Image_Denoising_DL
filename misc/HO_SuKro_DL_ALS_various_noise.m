function [D,D_structured,normVec, X] = HO_SuKro_DL_ALS_various_noise(params)
...
%   XtX = sum(Y.^2);
  ...
  for k_im = 1:length(params.data)
    XtX{k_im} = sum(params.data{k_im}.^2);
  end
end
...
if isfield(params, 'my_omp_training') && params.my_omp_training
    ...
    for k_im = 1:length(thresh)
        for k_OMP = 1:size(params.data{k_im},2)
            % Structure OMP
            if isfield(params,'odct_factors') % uses only first summing term of dictionary
                [X{k_im}(:,k_OMP),~,~,~] = ompfunc_mine(D, params.data{k_im}(:,k_OMP), params.dictsize, D_ip(:,1),normVec.',params.blocksize,1, prod(params.blocksize),0,0,0,thresh(k_im));
            else
                [X{k_im}(:,k_OMP),~,~,~] = ompfunc_mine(D, params.data{k_im}(:,k_OMP), params.dictsize, D_ip,normVec.',params.blocksize,params.alpha, prod(params.blocksize),0,0,0,thresh(k_im));
            end
%             [X(:,k_OMP),~,~,~] = SolveOMP(D, Y(:,k_OMP), params.dictsize, prod(params.blocksize),0,0,0,thresh);
    ...
    for k_im = 1:length(thresh)                                                                        
        if (memusage < MEM_HIGH)
          X{k_im} = ompfunc(D,params.data{k_im},G,thresh(k_im),ompparams{:});
          ...
          if (codemode == CODE_SPARSITY)
            ...
            X{k_im} = ompfunc(D'*params.data{k_im},XtX{k_im},G,thresh(k_im),ompparams{:});
          end
...
Y = reshape(cell2mat(params.data), [n N]);
