function [y,D,D_not_normalized, normVec, exec_times, nz] = image_denoise_various_noise(params,msgdelta)
% This function is a variation of image_denoise from ksvdbox used for the
% low-rank experiments in HYperspectral data in which one single dictionary
% is trained for all columns of U (i.e. one dictionary for different images
% with different noise levels).
% The input data is given in a cell array containing one image per cell.
%
% params.x: 3D array (previously cell array) containing all images stacked in last dimension.
% params.sigma: array containing one noise level per image
%
% Generates:
%
% params.Edata : as an array of same size of params.sigma
% params.lambda : as an array of same size of params.sigma
%
...
% for k_im = 1:length(sigma)
%    params.Edata{k_im} = sqrt(prod(blocksize)) * sigma(k_im) * gain;   % target error for omp 
% end
...
params.data = cell(1,size(params.x,3));
params.data = cell(1,5);
params.Edata = sqrt(prod(blocksize)) * sigma(1:5) * gain;   % target error for omp 
% for imnum = 1:size(params.x,3)
for imnum = 1:5
    x = params.x(:,:,imnum);
%     x = params.x{imnum};
    ...
    ids = cell(p,1);
    if (p==1)
      ids{1} = reggrid(length(x)-blocksize+1, round(trainnum/size(params.x,3)), 'eqdist');
      ids{1} = reggrid(length(x)-blocksize+1, round(trainnum/5), 'eqdist');
    else
      [ids{:}] = reggrid(size(x)-blocksize+1, round(trainnum/5), 'eqdist');
    end
    params.data{imnum} = sampgrid(x,blocksize,ids{:});

    % remove dc in blocks to conserve memory %
    dc_blocksize = 2000;
    for i = 1:dc_blocksize:size(params.data{imnum},2)
      blockids = i : min(i+dc_blocksize-1,size(params.data,2));
      params.data{imnum}(:,blockids) = remove_dc(params.data{imnum}(:,blockids),'columns');
    end
...
elseif params.algo_type == 2
    ...
    [D, D_not_normalized, normVec, ~] = HO_SuKro_DL_ALS_various_noise(params);
elseif params.algo_type == 3
    if (msgdelta>0)
        disp('KSVD training...');
    end
    D = ksvd_various_noise(params);
    D_not_normalized = D;
...
%   [y,nz] = ompdenoise2(params,msgdelta);
  [y,nz] = ompdenoise2_various_noise(params,msgdelta); % Modif
elseif (p==3)
