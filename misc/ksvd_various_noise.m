function [D,Gamma,err,gerr] = ksvd_various_noise(params,varargin)
% params.Edata : is given by a array (doesn't work for params.Tdata). It
% generates the variable 'thresh' as an array as well.
% params.data is given by a cell
%
...
%   XtX = colnorms_squared(data);
  XtX = cell(size(params.data));
  for k_im = 1:length(params.data)
    XtX{k_im} = colnorms_squared(params.data{k_im});
  end
...
for iter = 1:iternum
  ...
  Gamma = cell(size(params.data));
  if isfield(params, 'my_omp_training') && params.my_omp_training
    % Using my homemade OMP functions for fair comparison (Cassio)
    for k_im = 1:length(thresh)
      for k_OMP = 1:size(data,2)
          [Gamma{k_im}(:,k_OMP),~,~,~] = SolveOMP(D, data(:,k_OMP), params.dictsize, prod(params.blocksize),0,0,0,thresh);
      end
      Gamma{k_im} = sparse(Gamma{k_im});
    end
    ...
    for k_im = 1:length(thresh)
        Gamma{k_im} = sparsecode(params.data{k_im},D,XtX{k_im},G,thresh(k_im));
    end
  end
  ...
  if (testgen)
    if isfield(params, 'my_omp_training') && params.my_omp_training
        % Using my homemade OMP functions for fair comparison (Cassio)
        GammaG = zeros(size(D,2),size(testdata,2));
        for k_OMP = 1:size(testdata,2)
            [GammaG(:,k_OMP),~,~,~] = SolveOMP(D, testdata(:,k_OMP), params.dictsize, prod(params.blocksize),0,0,0,thresh);
        end
    else % Original implementation  
        if (memusage >= MEM_NORMAL)
          G = D'*D;
